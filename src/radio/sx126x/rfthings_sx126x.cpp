...
rft_status_t rfthings_sx126x::init(rft_region_t region) {
	...
	pinMode(sx126x_hal.irq, INPUT_PULLDOWN);
	...
	sx126x_set_lora_sync_word(&sx126x_hal, lora_params.syncword >> 8);
...
rft_status_t rfthings_sx126x::send_lora(byte *payload, uint32_t payload_len, uint32_t timeout, void (*tx_func)()) {
	...
	// number of symbol to transmit in t(s): n = t / tS
	// n = t / 2^SF / BW = t * BW / 2^SF 
	if (lora_params.symbol_time != 0) {
		symbol_len = 6 + lora_params.symbol_time * sx126x_get_lora_bw_in_hz(map_bandwidth(lora_params.bandwidth)) / ((1 << map_spreading_factor(lora_params.spreading_factor)) * 1e3);
	}
...
void rfthings_sx126x::irq_relay(void) {
...
rft_status_t rfthings_sx126x::relay(byte *payload, uint32_t payload_len, void (*rx_func)(), void (*sleep_func)()) {
	...
	// time to transmit one symbol: tS = 2^SF / BW
	sx126x_irq_mask_t irq_status;
	uint16_t symbol_len = 6 + lora_params.symbol_time * sx126x_get_lora_bw_in_hz(map_bandwidth(lora_params.bandwidth)) / ((1 << map_spreading_factor(lora_params.spreading_factor)) * 1e3);
	uint32_t detect_time = 1e3 * lora_params.detect_symbol * (1 << map_spreading_factor(lora_params.spreading_factor)) / sx126x_get_lora_bw_in_hz(map_bandwidth(lora_params.bandwidth));
	...
	sx126x_set_rx_duty_cycle(&sx126x_hal, detect_time, lora_params.symbol_time - detect_time);
...
rft_status_t rfthings_sx126x::send_uplink(byte *payload, uint32_t payload_len, void (*tx_func)(), void (*rx_func)()) {
	// buld LoRaWAN packet
